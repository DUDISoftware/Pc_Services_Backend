...
const createService = async (req, res, next) => {
  const schema = Joi.object({
    ...
    category: Joi.string().pattern(OBJECT_ID_RULE).message(OBJECT_ID_RULE_MESSAGE).required() // ✅ bắt buộc có category
  });
...
const updateService = async (req, res, next) => {
  const schema = Joi.object({
    ...
    category: Joi.string().pattern(OBJECT_ID_RULE).message(OBJECT_ID_RULE_MESSAGE).optional() // ✅ cho phép update category
  });
